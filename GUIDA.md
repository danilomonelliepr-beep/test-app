...
chi ha risposto — e per riprendere basta **trascinarlo nel campo dei file**
insieme, o al posto, dei sorgenti: l'applicazione lo riconosce dal contenuto e
riparte da dove si era. Vale anche per i file
salvati con le versioni precedenti dell'applicazione.

## Se il sorgente è andato perso

Si può caricare direttamente l'eseguibile: l'applicazione lo esamina e ne
ricava un resoconto — com'è stato costruito, contro quali librerie è linkato,
che stringhe contiene (query SQL, connection string, percorsi, messaggi) — che
poi viene analizzato come se fosse codice. E se il sorgente è stato ricostruito
con un decompilatore, l'applicazione **se ne accorge da sola** e si comporta di
conseguenza: i nomi che un decompilatore inventa (`FUN_00401a30`, `uVar7`) non
significano niente, quindi il modello viene avvertito di ignorarli e nessuna
riga che nasce da quei file può dichiararsi certa. In copertina del documento
è scritto quanti file sono di questo tipo.

Quanto si recupera dipende da come il programma è stato compilato: da un .NET o
da un Java il sorgente torna quasi per intero, da codice nativo si ottiene
codice che fa la stessa cosa ma senza nomi né commenti.

## Usare la chiave Azure aziendale

L'applicazione si apre già pronta per la risorsa Azure aziendale: provider
**Microsoft Azure OpenAI**, l'endpoint della risorsa e il deployment
**gpt-6-luna** sono già scritti. Manca solo la chiave.

**Su Streamlit Cloud** la chiave si mette una volta sola nei *Secrets*
dell'applicazione (*Manage app → Settings → Secrets*):

```toml
AZURE_OPENAI_API_KEY = "la-chiave-aziendale"
```

Da quel momento l'applicazione si apre con tutto compilato: si carica il
codice e si analizza. **In locale** la stessa cosa si fa con una variabile
d'ambiente con lo stesso nome. **La chiave non va mai scritta nel codice né
messa su git.**

Per controllare che tutto risponda c'è **Check the connection**, sotto la
chiave. Per usare altri modelli oltre a gpt-6-luna si aggiungono nel campo
**Deployments**, separati da virgola e nell'ordine in cui provarli — per
esempio `gpt-6-luna, gpt-5.6-luna`: il primo lavora, gli altri sono la riserva.
Si possono anche fissare nei Secrets con `AZURE_DEPLOYMENT`, e l'endpoint con
`AZURE_OPENAI_ENDPOINT`, se un giorno cambiasse.

**Quali modelli ci sono.** Li decide chi gestisce la risorsa. Oggi, secondo lo
snippet, `gpt-6-luna` e `gpt-5.6-luna` per il testo; `gpt-image-2` genera
immagini e l'applicazione lo esclude da sola, perché non legge codice. Se la
chiave ha il permesso di elencare i deployment, l'applicazione li trova da sé;
altrimenti — il caso più comune con una chiave aziendale — si scrivono a mano.
Per un modello nuovo va chiesto il deployment a chi gestisce la risorsa, e poi
aggiunto all'elenco.

**Quanto farlo ragionare** (*Thinking time*, in *Expert settings*). Tre
gradini: Fast è `low`, Balanced `medium`, Deep `high`. Balanced è il
predefinito e il punto di partenza giusto per leggere codice legacy; su
`gpt-5.6-luna`, che lo snippet indica per lo sforzo alto, conviene Deep. I livelli `xhigh` e `max`
esistono ma l'applicazione non li offre: su una risposta lunga come la nostra
vogliono dire minuti per ogni lotto. Il ragionamento consuma lo stesso tetto
della risposta, e l'applicazione gli riserva un margine da sola.

**Quanti lotti in parallelo** (*Batches at once*). Su Azure il limite è **per
deployment**, in token al minuto (TPM) e richieste al minuto: si legge nel
portale Azure AI Foundry, alla voce del deployment, o lo si chiede a chi
gestisce la risorsa. Conta che Azure, per decidere se accettare una richiesta,
stima i token **prima** di eseguirla, compreso il tetto della risposta: un lotto
pieno vale fra 50 000 e 70 000 token a seconda del ragionamento. A spanne: sotto
i 100 000 TPM un lotto alla volta; fra 100 000 e 300 000, due; sopra, tre o
quattro. Se si esagera non si rompe niente: il 429 di Azure viene aspettato e
riprovato **sullo stesso modello**, costa solo tempo.

**Cosa resta sulla risorsa.** Ogni chiamata parte con `store: false`: la
risposta non viene conservata sul server per essere ripresa, come invece fa la
Responses API di serie. Il sorgente del cliente passa, non resta.
...
| `test_catena.py` | 227 casi, senza rete, senza chiavi, senza costi |
| `avvia.py` | Installa quello che manca, configura Streamlit e apre l'applicazione |
| `binary_triage.py` | Esame di un eseguibile di cui manca il sorgente |
...
modello. Tetti: 2 MB per file di testo, 50 MB per un eseguibile (il resoconto che ne esce ha comunque dimensione fissa), 1,5 M caratteri in totale.
...
| Azure OpenAI | Responses API (`/openai/v1/responses`): `text.format = json_object`; sul dialetto classico `response_format` |
...
Deep) diventa `thinkingLevel` su Gemini, `thinking` con budget su
...
**le proporzioni lette dall'immagine stessa** (intestazione PNG o JPEG, altrimenti
PIL); non esiste una misura fissa di ripiego.
...
`python test_catena.py` — 227 casi, nessuna rete, nessuna chiave, nessun costo.
