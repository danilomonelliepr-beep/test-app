...
You can upload the executable itself: the application examines it and produces
a report — how it was built, which libraries it links against, which strings it
contains (SQL, connection strings, paths, messages) — which is then analysed as
if it were code. And if the source was reconstructed with a decompiler, the
application **notices by itself**: the names a decompiler invents
(`FUN_00401a30`, `uVar7`) mean nothing, so the model is told to ignore them and
no row coming from those files may claim high confidence. The document says on
its cover how many files are of this kind.

How much can be recovered depends on how the program was compiled: from a .NET
or a Java binary the source comes back almost whole; from native code you get
code that does the same thing, without real names or comments.

## Using the company Azure key

The application opens ready for the company Azure resource: provider
**Microsoft Azure OpenAI**, the resource endpoint and the **gpt-6-luna**
deployment are already filled in. Only the key is missing.

**On Streamlit Cloud** the key goes once into the app's *Secrets*
(*Manage app → Settings → Secrets*):

```toml
AZURE_OPENAI_API_KEY = "the-company-key"
```

From then on the application opens with everything filled in: upload the code
and analyse. **Locally** the same is done with an environment variable of the
same name. **The key never goes in the code or on git.**

To check that everything answers there is **Check the connection**, under the
key. To use other models besides gpt-6-luna, add them in the **Deployments**
field, comma-separated and in the order to try them — for example
`gpt-6-luna, gpt-5.6-luna`: the first does the work, the others are the
fallback. They can also be fixed in the Secrets with `AZURE_DEPLOYMENT`, and the
endpoint with `AZURE_OPENAI_ENDPOINT`, should it ever change.

**Which models exist.** Whoever manages the resource decides. Today, according
to the snippet, `gpt-6-luna` and `gpt-5.6-luna` for text; `gpt-image-2`
generates images and the application leaves it out by itself, since it does not
read code. If the key may list the deployments, the application finds them
itself; otherwise — the usual case with a company key — you type them. For a
new model, ask the resource owner to deploy it, then add it to the list.

**How much to let it reason** (*Thinking time*, under *Expert settings*). Three
steps: Fast is `low`, Balanced `medium`, Deep `high`. Balanced is the default
and the right starting point for reading legacy code; on `gpt-5.6-luna`, which
the snippet points at high effort, use Deep. The `xhigh` and `max` levels exist but
the application does not offer them: on an answer as long as ours they mean
minutes per batch. Reasoning uses the same budget as the answer, and the
application reserves a margin for it by itself.

**How many batches at once** (*Batches at once*). On Azure the limit is **per
deployment**, in tokens per minute (TPM) and requests per minute: it is shown in
the Azure AI Foundry portal under the deployment, or ask the resource owner.
Note that Azure estimates the tokens **before** running a request, including
the answer's ceiling: a full batch is worth 50,000 to 70,000 tokens depending on
reasoning. Roughly: under 100,000 TPM, one batch at a time; 100,000 to 300,000,
two; above that, three or four. Going too high breaks nothing: Azure's 429 is
waited out and retried **on the same model**, it only costs time.

**What stays on the resource.** Every call goes with `store: false`: the answer
is not kept on the server for later retrieval, as the Responses API does by
default. The customer's source passes through; it does not stay.
...
| `test_catena.py` | 227 cases, no network, no keys, no cost |
| `avvia.py` | Installs what is missing, configures Streamlit and opens the application |
| `binary_triage.py` | Examines an executable whose source is lost |
...
eighty known, the others marked "Unknown" and left to the model. Limits: 2 MB per file of text, 50 MB for an executable (the report it produces has a fixed size regardless), 1.5 M characters in total.
...
| Azure OpenAI | Responses API (`/openai/v1/responses`): `text.format = json_object`; on the classic dialect `response_format` |
...
· Deep) becomes `thinkingLevel` on Gemini, `thinking` with a budget
...
dependencies by reliability. Images honour **the proportions read from the image itself** (PNG or JPEG
header, otherwise PIL); there is no fixed fallback size.
...
`python test_catena.py` — 227 cases, no network, no key, no cost. The provider
