...
Versione contratto JSON: **2.1** · Collaudo: `python test_catena.py` → 227/227 · Aggiornato al 23 settembre 2026
...
**AC** i diagrammi schiacciati, tornati,
**AD** quando il sorgente non c'è,
**AE** il resoconto che analizzava sé stesso,
**AF** tre falle nel meccanismo di fiducia,
**AG** il tetto sbagliato per gli eseguibili,
**AH** la sigla nel titolo,
**AI** il banner,
**AJ** la chiave Azure aziendale,
**AK** l'app si apre già compilata.
...
| C15 | **Collaudo automatico `test_catena.py`** — 227 casi, nessuna rete, nessuna chiave, nessun costo: scoperta, scalata, memoria, messaggi, tetti, riparazione JSON, enum, Mermaid, lotti. | Porting del collaudo di Nuvia. È il modo per cambiare una regola e sapere subito cosa si rompe. |
...
## AC · I diagrammi schiacciati, tornati (e perché nel browser no)

Segnalazione: lo schiacciamento dei diagrammi nel PDF e nel Word, corretto nel
blocco D, è tornato. Nel browser no.

**La causa.** La correzione del blocco D leggeva le dimensioni dall'intestazione
**PNG**. Sul deploy del collega non c'è `mmdc`, quindi i diagrammi arrivano dal
servizio esterno mermaid.ink — che di serie restituisce un **JPEG**. La lettura
falliva in silenzio e si ricadeva sulla misura fissa di ripiego, 720×240: cioè
esattamente il 3:1 del difetto originale. Nel browser va bene perché lì il
diagramma lo disegna la pagina stessa, senza passare da un'immagine.

| # | Modifica | Perché |
|---|---|---|
| AC1 | **Al servizio esterno si chiede esplicitamente un PNG** (`?type=png`). | Il formato non deve dipendere dal valore di serie di un servizio altrui. |
| AC2 | **Le dimensioni si leggono da PNG, da JPEG e, per il resto, tramite PIL** (`dimensioni_immagine`; il nome vecchio `dimensioni_png` resta e funziona). | Anche se un giorno il servizio restituisse un altro formato, o si usasse un altro servizio. |
| AC3 | **La misura fissa di ripiego è sparita.** Se le dimensioni non si leggono, `misura_per_riquadro` torna `None` e l'esportatore le chiede a ReportLab stesso, che le legge dall'immagine. In nessun caso si torna a un numero fisso. | Un ripiego che riproduce il difetto non è un ripiego: è il difetto con un altro nome. |
| AC4 | **Tre casi in più nel collaudo**: le dimensioni lette da un JPEG, il diagramma alto arrivato come JPEG che non viene schiacciato, e la richiesta esplicita del PNG. Provato anche sul PDF finito con un JPEG 800×2000: esce 1:2,48. | 158 in tutto. Nel blocco D il collaudo usava solo PNG: il buco era lì. |

---

## AD · Quando il sorgente non c'è: eseguibili e codice decompilato

Due cose nuove, costruite per non toccare niente di quello che funziona: un
modulo autonomo e un gancio che nel contratto esisteva già. **La versione del
contratto resta 2.1**: nessun campo nuovo, i JSON salvati restano validi.

### `binary_triage.py` — esaminare un eseguibile

| # | Cosa | Perché |
|---|---|---|
| AD1 | **Modulo nuovo e indipendente**: nessun altro file lo importa se non `app.py`, e funziona anche da solo (`python binary_triage.py PROGRAMMA.EXE`). Solo libreria standard, più `pefile` se c'è. | Una cosa nuova non deve poter rompere una cosa che funziona. |
| AD2 | **Dice in che caso si è**, che è la domanda che conta: .NET e Java → i nomi sono dentro il file, ILSpy o CFR danno codice quasi identico all'originale, spesso ricompilabile (è recupero, non ricostruzione); PyInstaller → si estrae e si decompila il bytecode; VB6 in p-code → si recupera parecchio, form comprese; nativo (C, C++, Delphi) → Ghidra dà codice *funzionalmente equivalente*, non il sorgente. | Sbagliare il caso significa usare lo strumento sbagliato per mezza giornata. |
| AD3 | **Firme a punteggio, non a parola singola.** Servono più indizi concordi e il formato giusto. | Con le firme deboli `objdump` su Linux veniva riconosciuto come Delphi, perché conosce i formati Borland. Una diagnosi sbagliata qui è peggio di nessuna diagnosi. |
| AD4 | **Segnala il percorso del `.pdb`** se il binario lo dichiara. | Con i simboli di compilazione la qualità della decompilazione fa un salto: vale la pena cercarli prima di cominciare. |
| AD5 | **Librerie linkate spiegate** (`OCI.dll` → Oracle, `WININET` → HTTP) e **stringhe per categoria**: SQL, connection string, percorsi, file, credenziali, messaggi. | Anche senza decompilare, un binario dice già cosa fa e con chi parla. |

### La provenienza: sorgente, decompilato, resoconto

| # | Cosa | Perché |
|---|---|---|
| AD6 | **Un eseguibile caricato non viene più respinto**: diventa il resoconto del suo esame, che è testo e l'estrattore lo legge come tutto il resto. Un binario che eseguibile non è resta respinto, con il rimedio. | Quando il sorgente è perso è l'unica cosa che resta. |
| AD7 | **Il codice decompilato si riconosce da solo** — `FUN_00401a30`, `sub_4012F0`, `uVar7`, `DAT_…`, le note di Ghidra, le forme di Hex-Rays. Servono almeno tre segni **diversi**: un file vero può contenere `sub_`, tre marche di decompilatore no. | Nessuno deve ricordarsi di spuntare una casella. |
| AD8 | **Il modello viene avvertito, nominando i file.** I nomi inventati dal decompilatore non vanno mai presentati come nomi di componenti o tabelle, e su di essi la confidenza scende. La nota passa dal parametro `note` che `prompt_analisi` aveva già. | Nessun campo nuovo nel contratto, nessuna versione da cambiare. |
| AD9 | **Nessuna riga nata da codice decompilato o da un resoconto può dichiararsi `HIGH`**: il tetto lo mette il codice, e nell'evidenza compare «from decompiled code». | Il modello non ha modo di sapere che `FUN_00401a30` non è un nome vero; il codice lo sa. Una riga `HIGH` in un documento che qualcuno firma vale come un fatto. |
| AD10 | **Si vede prima di partire** (una marca nella riga di stato e un avviso con i nomi dei file) **e dopo, in copertina del documento**: quanti file sono decompilati, quanti sono resoconti. | Chi riceve il PDF deve saperlo dalla prima pagina, non scoprirlo a pagina otto leggendo un nome che nel programma non esiste. |
| AD11 | **Sedici casi in più nel collaudo**, e uno vecchio aggiornato (una DLL ora si esamina invece di essere respinta). `pefile` aggiunto alle dipendenze: piccolo, e senza di lui gli import di Windows non si leggono. | 174 in tutto. |

### Il limite, detto chiaro

Sul codice nativo un decompilatore non restituisce il sorgente perduto:
restituisce codice che fa la stessa cosa, senza nomi veri e senza commenti.
Va bene per capire, non per ricompilare. Sul .NET e su Java invece il sorgente
si recupera quasi per intero — ed è il caso in cui si trova chi lavora con
`.vb`.

---

## AE · Il resoconto di un binario che analizzava sé stesso

Segnalazione dal campo: nella tabella delle dipendenze di un `.exe` comparivano
righe con bersaglio `LIBRARIES`, `FUNCTIONS`, `PERCORSO` — nomi senza senso,
e uno persino in italiano dentro un documento inglese.

**La causa era meccanica, non il modello.** Il resoconto dell'esame binario è
testo che l'app stessa genera, con intestazioni come `LINKED LIBRARIES
(what the program talks to)` e `STRINGS — PERCORSO (5 found)`. Quel resoconto
torna poi in pipeline **come se fosse codice sorgente** — è così che il modello
lo legge — e prima ci passava sopra anche l'analisi statica: il pattern
generico «una parola seguita da parentesi» (quello che cerca le chiamate a
funzione nei linguaggi senza un parser dedicato) prendeva `LIBRARIES (` e
`PERCORSO (` dalle intestazioni del referto e li registrava come dipendenze
sospette. Il modello non ha inventato niente: ha ricevuto il dato già sporco.
Confermato isolando la regex e facendola girare sul resoconto da sola.

| # | Modifica | Perché |
|---|---|---|
| AE1 | **La ricerca generica di dipendenze non gira più sui resoconti d'esame.** Un file con provenienza `esame` è prosa che descrive un binario, non codice: cercarci chiamate a funzione non ha senso, e quelle vere (le DLL importate) sono già catturate altrove con un pattern esplicito che non soffre di questo problema. | Non serviva ripulire il sintomo (filtrare `LIBRARIES` per nome): la causa era che si stava analizzando come codice un testo che codice non è. |
| AE2 | **Le categorie di stringhe in `binary_triage.py` sono in inglese** (`connessione`→`connection string`, `indirizzo`→`address`, `percorso`→`path`, `credenziale`→`credential`, `messaggio`→`message`). | Anche a difetto corretto, restava sbagliato avere parole italiane in un documento che si consegna in inglese. |
| AE3 | **I rischi restano rilevati**: un `SELECT` vero trovato nelle stringhe di un binario continua a generare la riga di rischio (è successo davvero nel caso segnalato, `TR-001 DYNAMIC_SQL`, ed è un fatto corretto). Solo la ricerca di chiamate a funzione è stata spenta sui resoconti, non tutta l'analisi statica. | Un segnale vero non va tolto insieme al rumore: SQL incorporato in un binario è un rischio reale da documentare. |
| AE4 | **Cinque casi in più nel collaudo**, incluso uno che riproduce esattamente la segnalazione (un `.exe` finto con una query SQL dentro, verificato che non produca più `LIBRARIES`/`FUNCTIONS`/`PERCORSO` come dipendenze) e uno che dimostra la causa isolando la regex. | 179 in tutto. |

### Sull'altro screenshot: l'errore 404 grezzo su `gemini-2.5-flash`

Quel messaggio — `404 NOT_FOUND. {'error': {'code': 404, ...`, nella forma
prodotta dall'SDK ufficiale di Google — non può venire dalla catena attuale:
da agosto il trasporto è `requests` diretto (blocco A16), che intercetta il
404 esplicitamente, lo traduce nel messaggio in italiano/inglese, **e fa
scalare la catena al modello successivo invece di fermarsi**. È quasi certo
che quella schermata venga da una build precedente al riscritto di
`model_chain.py` — l'interfaccia nell'immagine (campo di testo libero
«Gemini Model», niente tappe numerate, niente «Check the connection») è
quella di prima del rifacimento dell'interfaccia. Non è stato toccato nulla
per questo: il comportamento che quella schermata mostra come mancante è
esattamente quello che la catena attuale fa di serie.

---

## AF · Tre falle nel meccanismo di fiducia, trovate rileggendo un PDF vero

Non ipotizzate: riprodotte una per una nel codice, prima di correggerle. Tutte
e tre toccano la parte più delicata dell'applicazione — la distinzione fra
«fatto» e «interpretazione» che tutto il resto dà per scontata.

| # | Difetto | Correzione |
|---|---|---|
| AF1 | **Il tetto di confidenza non raggiungeva `data_flows`, `application_mapping` né `impact_analysis`.** Guardava solo due nomi di campo (`source_file`, `affected_component`), e quelle tre sezioni usano nomi diversi (`source`/`target`; `source_component`; `affected_components`, al plurale). Righe nate al 100% da un file d'esame restavano a `HIGH` — proprio quello che il banner dell'app promette non succeda: nel PDF segnalato, DF-001, DF-002, AM-001 e IA-001 mostravano tutte `•••` nonostante venissero da un solo `.exe` senza sorgente. | Ora si guarda OGNI valore della riga, non due nomi fissi: se il nome del file compare da qualche parte nella riga, la riga eredita il tetto. Riprodotto e corretto per tutte e tre le sezioni. |
| AF2 | **Il modello può dichiararsi «parser» da solo.** `source` è un campo del contratto come un altro, e niente controllava che un valore `STATIC_ANALYSIS` scritto dal modello corrispondesse a qualcosa che il parser avesse davvero prodotto. Il documento poi mostra «Found by: parser» — l'etichetta che dice «un fatto che non può sbagliare» — per una riga che il fatto non ce l'ha. | **`forza_provenienza_modello()`**: prima che la risposta del modello incontri quella del parser, il campo `source` di `components` e `technical_risks` viene azzerato a `LLM_ANALYSIS`, qualunque cosa il modello ci abbia scritto. Solo la fusione con l'analisi statica vera, più avanti nella pipeline, ha ancora il permesso di scrivere `STATIC_ANALYSIS` — verificato che quella via resti intatta. |
| AF3 | **La copertina del PDF contava solo `technical_risks`.** `impact_analysis` ha anch'essa un campo `severity`, e una riga `CRITICAL` lì non veniva contata: la copertina diceva «0», il documento stesso ne mostrava una tre pagine dopo, e l'indicatore a schermo (che già scorre tutte le sezioni) diceva «1». Stesso documento, due numeri in disaccordo. | La copertina ora conta come l'interfaccia: qualunque riga con `severity` a `CRITICAL` o `HIGH`, in qualunque sezione. |
| AF4 | **Sette casi in più nel collaudo**, uno per ciascuna sezione toccata dal primo difetto, uno per il secondo con verifica che la via legittima (una riga davvero dal parser) resti intatta, due per il terzo. | 186 in tutto. |

---

## AG · Il tetto sbagliato per gli eseguibili (segnalazione: «PrIDE.dll exceeds the allowed size»)

| # | Difetto | Correzione |
|---|---|---|
| AG1 | **Una `.dll` vera veniva respinta prima ancora di essere riconosciuta come eseguibile.** Il limite di dimensione (2 MB) veniva controllato PRIMA della firma del file, e quel limite è pensato per il testo sorgente — che finisce per intero nella chiamata al modello. Un eseguibile non ci finisce mai per intero: se ne estrae un resoconto di dimensione fissa (intestazione, import, un tetto di stringhe), quindi la sua dimensione reale non dovrebbe contare quanto quella di un file di codice. Una `.dll` vera è spesso più pesante di un singolo `.cs`, e finiva bloccata dal tetto sbagliato. | **Le firme si guardano prima del limite.** Un eseguibile riconosciuto (`.exe`, `.dll`, e in generale MZ/PE) ora ha un tetto proprio, molto più alto: **50 MB**, lo stesso limite che Streamlit impone già al caricamento — oltre quella soglia il file non arriva nemmeno al codice, quindi renderlo più permissivo non servirebbe a niente. Provato con una `.dll` da 8 MB (accettata) e una da 55 MB (respinta, con il numero giusto nel messaggio). |
| AG2 | Il testo d'aiuto del caricatore diceva ancora **«Binary files are refused»** — rimasto indietro da quando gli eseguibili hanno iniziato a essere esaminati invece che respinti (blocco AD, 17 settembre). | Aggiornato: dice che gli eseguibili vengono esaminati, con il loro tetto di 50 MB, distinto da quello del resto. |
| AG3 | **Tre casi in più nel collaudo**: la DLL da 8 MB che ora entra, il tetto più alto che resta un tetto vero, e conferma che il limite sul testo sorgente non è cambiato. | 189 in tutto. |
...
| # | Modifica | Perché |
|---|---|---|
| AH1 | **Il titolo è «LAKE: Legacy Application Knowledge Extractor»**: la sigla davanti, in evidenza, e il nome esteso che la spiega — con le iniziali delle quattro parole nello stesso colore, così il legame fra sigla e nome si vede senza doverlo dire. Anche il titolo della scheda del browser porta la sigla. | Richiesto: il prodotto lo chiamano LAKE. |
| AH2 | **`ui._con_iniziali_in_evidenza()` è generica**: riconosce la forma «SIGLA: nome esteso» e, in mancanza dei due punti, evidenzia le sole iniziali. Non è legata a queste parole: cambiando il nome, cambia da sé quello che viene evidenziato. | Lo stesso principio di tutto il resto: il codice non deve sapere cose che può scoprire da solo. |
| AH3 | **Quattro casi nel collaudo**: la sigla evidenziata per intero, le iniziali che la richiamano, il resto delle parole intatto, e la forma senza due punti. | 193 in tutto. |

**Nota per chi legge questo file**: durante questa modifica un errore mio ha
svuotato `app.py` per un istante (uno script scritto male, con un
`open(...,'w').close()` senza contenuto). Se ne sono accorti subito i
controlli — `wc -l` a zero, poi `py_compile` fallito — ed è stato ripristinato
dalla copia già verificata un momento prima (`/tmp/pulito`), non riscritto a
memoria: il collaudo a 189/189 confermava che il ripristino fosse esatto
*prima* di procedere con la modifica vera. Lo registro qui perché fa parte
della cronologia, non perché sia successo qualcosa all'utente: nessun file
del pacchetto consegnato ne ha risentito.
...
| # | Modifica | Perché |
|---|---|---|
| AI1 | **Il banner sostituisce il titolo scritto** in cima alla pagina (`assets/banner-lake.jpg`). | Fornito. |
| AI2 | **Il banner è un SVG disegnato sui colori del tema**, non un'immagine generata. Pesa **2,6 KB** contro i 688 KB di partenza, resta nitido a qualunque ingrandimento e su qualunque schermo, e il testo dentro è testo vero (selezionabile, nitido in stampa). Il fondo va da `#0E434C` a `#15616D` — l'accento dell'applicazione e la sua variante cupa, gli stessi valori che stanno in cima a `ui.py`. | Un'immagine che *somiglia* ai colori dell'app invecchia male: al primo ritocco del tema stona. Un SVG che usa gli stessi valori si allinea cambiando una riga. |
| AI3 | **CORRETTO: il banner sta DENTRO `ui.py`**, non in un file accanto. In produzione la cartella `assets/` non è arrivata e la testata è tornata al titolo scritto — lo stesso identico modo in cui si era persa `.streamlit/`. Un modulo Python invece arriva sempre, perché senza non parte niente. | Due volte lo stesso difetto è un difetto di progetto, non sfortuna: quello che serve a far funzionare l'applicazione non deve stare in un file che si può perdere per strada. |
| AI3b | **Scritto inline nella pagina**, non come `data:` dentro un `<img>`. | Il ripulitore di HTML del browser tratta le due forme in modo diverso; l'SVG scritto direttamente nella pagina è quella che passa sempre. Verificato in un browser vero. |
| AI4 | **Il titolo resta scritto nella pagina**, fuori dallo schermo ma leggibile dagli assistivi (`.solo-lettori`, non `display:none`, che lo toglierebbe anche a loro). | Chi ascolta la pagina invece di guardarla deve sentire il nome del prodotto. |
| AI5 | **Cinque casi nel collaudo**: il banner dentro il modulo e nessuna cartella `assets/`, il peso sotto i 20 KB, i colori del tema, il titolo per gli assistivi, l'SVG presente nella testata. | 198 in tutto. |

Nota: i documenti PDF e Word continuano a usare il nome scritto, non il
banner — un'immagine scura a piena larghezza in cima a un documento da
stampare sarebbe sbagliata.

---

## AJ · La chiave Azure aziendale (snippet `llm-usage-snippets`)

Lo snippet usa la **Responses API** di Azure OpenAI sull'endpoint
`{risorsa}/openai/v1/`, con l'SDK `openai`. La catena parlava con Azure solo
nel dialetto classico (`/openai/deployments/{d}/chat/completions?api-version=…`)
e **il provider Azure non aveva mai avuto un collaudo, nemmeno con un finto**:
nessuno dei 198 casi lo toccava.

| # | Modifica | Perché |
|---|---|---|
| AJ1 | **La chiamata usa la Responses API**, come nello snippet: `instructions` al posto del messaggio di sistema, `input`, `reasoning.effort`, `max_output_tokens`, `text.format = json_object`. Il testo si legge dagli elementi `message` di `output`, non da quelli di ragionamento. | È il dialetto della risorsa aziendale, e quello nuovo di Azure. |
| AJ2 | **Trasporto `requests` diretto, non l'SDK `openai`** dello snippet. Il contenuto della richiesta è lo stesso; cambia solo chi la spedisce. | Stessa ragione per cui non si usano gli SDK degli altri provider (blocco A16): l'SDK riprova i 429 per conto suo, sopra la catena, e nasconde il codice HTTP che serve a decidere. |
| AJ3 | **`store: false`** su ogni chiamata. | Di serie la Responses API conserva la risposta sul server per poterla riprendere con `previous_response_id`. A noi non serve, e il prompt contiene il sorgente del cliente: non deve restare su una risorsa condivisa oltre la chiamata. La continuazione di una risposta tagliata si fa con lo stato sul client — la seconda forma dello snippet. |
| AJ4 | **I livelli di ragionamento tradotti**: Fast → `none`, Balanced → `low`, Thorough → `medium`, Deep → `high`. Se un modello rifiuta `none`, si sale da soli a `low` e lo si ricorda (stessa scala degli altri provider). | Lo snippet elenca `none · low · medium · high · xhigh · max`. `xhigh` e `max` non sono esposti: su una risposta da migliaia di righe di JSON vogliono dire minuti per lotto. |
| AJ5 | **Un margine per i token di ragionamento dentro `max_output_tokens`** (+4 000 a `low`, +8 000 a `medium`, +16 000 a `high`). | Nella Responses API il ragionamento si conta DENTRO il tetto della risposta: senza margine, a sforzo alto il modello finisce lo spazio pensando e restituisce una risposta vuota o tagliata. |
| AJ6 | **CORRETTO: un 429 di Azure non fa più cambiare modello.** Su Azure quel codice è quasi sempre il limite di token o richieste al minuto del deployment: passeggero, e il provider dice quanto aspettare (`Retry-After`). Ora si aspetta quel tempo (fino a 20 s) e si riprova lo stesso modello; si cambia solo se il messaggio dice che il credito è finito davvero (`insufficient_quota`). | Prima valeva «quota», che fa scendere la catena: con più lotti in parallelo, lotti diversi sarebbero stati scritti da modelli diversi. |
| AJ7 | **I deployment si scrivono al passo 1**, accanto all'endpoint, anche più d'uno separati da virgola: diventano la catena, nell'ordine scritto (`gpt-6-luna, gpt-5.6-luna`). Legge anche `AZURE_DEPLOYMENT`, il nome di variabile dello snippet. | Su Azure il nome che si chiama è quello dato da chi ha creato la risorsa, e con una chiave aziendale l'elenco di solito non si può leggere: senza nomi non si parte, quindi è un dato di connessione, non un'impostazione da esperto. |
| AJ8 | **CORRETTO, trovato dal collaudo prima di arrivare in produzione**: con due deployment scritti, la catena chiamava un modello di nome letterale `"gpt-6-luna, gpt-5.6-luna"` — il «modello preferito» prendeva l'intera stringa con la virgola. Ora è il primo nome. | Nessun modello si chiama così: ogni analisi sarebbe fallita con «deployment non trovato». |
| AJ9 | **«Check the connection» prova i deployment scritti**, non dei nomi generici. | Prima provava `gpt-4o`, che sulla risorsa aziendale non esiste. |
| AJ10 | **La scoperta dei deployment prova due versioni dell'API** (`2023-03-15-preview`, `2022-12-01`), le ultime che espongono l'elenco nel piano dati. Esclude i modelli che non servono a leggere codice (`gpt-image-2`, embedding, audio). Le generazioni nuove vanno in testa: `gpt-6-luna` prima di `gpt-5.6-luna`. | Prima un `gpt-6` non corrispondeva a nessuna fascia e finiva in fondo, dietro a un `gpt-4o`. |
| AJ11 | **Ripiego automatico sul dialetto classico** se la risorsa non ha `/openai/v1` (404 sul percorso, non sul deployment), ricordato per quel modello. | Così la stessa applicazione va sia sulla risorsa aziendale sia su una più vecchia. |
| AJ12 | L'endpoint incollato con `/openai/v1/` in fondo, come nello snippet, viene ripulito: il percorso lo mette l'applicazione. | Evita un `…/openai/v1/openai/v1/responses`. |
| AJ13 | **Ventidue casi di collaudo nuovi** con un Azure finto che risponde nel formato vero della Responses API: forma della richiesta, `store`, margine, JSON, catena scritta a mano, scoperta, livelli, rifiuto di `none`, 429 con attesa sullo stesso modello, credito finito, risposta tagliata e continuazione, ripiego classico e sua memoria, filtro dei contenuti. | 220 in tutto. **Nessuno parla con l'Azure vero**: la prima prova vera è «Check the connection» con la chiave aziendale. |

---

## AK · L'app si apre già compilata, e tre gradini di ragionamento

| # | Modifica | Perché |
|---|---|---|
| AK1 | **Tre gradini di ragionamento invece di quattro**: Fast → `low`, Balanced → `medium`, Deep → `high`. «Thorough» non c'è più. Balanced resta il predefinito, quindi il predefinito sale da `low` a `medium`. Vale per tutti i provider. | Deciso. Il livello più basso di tutti (`minimal`, su Azure `none`) resta solo per la prova di contatto, dove serve una risposta da due parole. Sostituisce la tabella dei livelli del blocco AJ4. |
| AK2 | **L'app si apre su Azure con l'endpoint aziendale e `gpt-6-luna` già scritti.** Chi vuole altri deployment li aggiunge a mano nello stesso campo, separati da virgola. | L'endpoint e il nome del deployment non sono segreti — un indirizzo e un nome — e possono stare nel codice (`AZURE_ENDPOINT_AZIENDALE`, `AZURE_DEPLOYMENT_AZIENDALE`, in testa alla barra laterale). |
| AK3 | **La chiave NON è nel codice, e non ci va.** Arriva dall'ambiente o dai Secrets di Streamlit Cloud: se c'è, il campo si apre compilato anche lui. | Una chiave nel codice finisce su git, e da lì dappertutto. |
| AK4 | **`impostazione()` legge ambiente E Secrets di Streamlit**, per tutti i provider, in quest'ordine, con il valore predefinito in fondo. | Streamlit Cloud espone i secrets di primo livello anche come variabili d'ambiente, ma leggerli direttamente non costa niente e non dipende da quel comportamento. Provato con Streamlit vero e un file di secrets: la chiave compare nel campo. |
| AK5 | **Sette casi in più nel collaudo**: i tre gradini, il predefinito `medium`, endpoint e deployment all'apertura, la chiave assente dal codice, l'ambiente che vince sul predefinito, il predefinito senza errori quando mancano ambiente e secrets. | 227 in tutto. |
