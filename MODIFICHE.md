...
Versione contratto JSON: **2.1** · Collaudo: `python test_catena.py` → 197/197 · Aggiornato al 18 settembre 2026
...
| C15 | **Collaudo automatico `test_catena.py`** — 197 casi, nessuna rete, nessuna chiave, nessun costo: scoperta, scalata, memoria, messaggi, tetti, riparazione JSON, enum, Mermaid, lotti. | Porting del collaudo di Nuvia. È il modo per cambiare una regola e sapere subito cosa si rompe. |
...
| AI3 | **Incorporato nella pagina come `data:`**, letto e codificato una volta sola (`lru_cache`). | Così viaggia con l'HTML della testata e non dipende da come Streamlit espone la cartella dei file statici, che cambia fra le versioni. |
| AI4 | **Il titolo resta come testo alternativo dell'immagine**, e se il file mancasse la testata torna da sé alla scritta «LAKE: Legacy…» con le iniziali colorate. | Il nome del prodotto non deve dipendere da un file che potrebbe non esserci, e chi usa uno screen reader deve leggerlo comunque. |
| AI5 | **Quattro casi nel collaudo**: il banner incorporato come SVG, il peso sotto i 20 KB, i colori del tema presenti nel file, il testo alternativo nell'HTML prodotto. | 197 in tutto. |
