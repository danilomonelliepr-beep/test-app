...
`AZURE_OPENAI_ENDPOINT` e `AZURE_DEPLOYMENT` (anche più d'uno, separati da
virgola: `gpt-6-luna, gpt-5.6-luna`). Per la risorsa Azure aziendale c'è una
sezione apposta nella guida.

## Metterlo su git

Ogni modulo porta una versione (`__version__`) e l'app all'avvio verifica che
coincidano: se si aggiorna un file solo, si ferma e dice quali sono vecchi.
Aggiornare sempre **tutta la cartella** dallo stesso pacchetto; su Streamlit
Cloud, riavviare l'app dopo il push.

Tutto il progetto va versionato così com'è. Non ci sono cartelle nascoste: il
tema di Streamlit lo imposta `app.py` da codice a ogni avvio (`ui.py`,
`imposta_tema_streamlit`), quindi vale ovunque, Streamlit Cloud compreso. Non
c'è niente da tenere fuori: le chiavi non sono nel codice e l'unico contenuto
generato (`cache/`) è già escluso.
