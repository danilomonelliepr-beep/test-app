...
# ═══ I VALORI CON CUI L'APP SI APRE ════════════════════════════════════════
# La risorsa Azure aziendale e il suo deployment sono noti: l'app si apre già
# compilata con quelli, e chi vuole altro lo scrive sopra. L'endpoint e il nome
# del deployment NON sono segreti — sono un indirizzo e un nome — quindi
# possono stare nel codice. LA CHIAVE NO: arriva dall'ambiente o dai Secrets
# di Streamlit Cloud, mai da qui.
AZURE_ENDPOINT_AZIENDALE = "https://2026-epr-hackathon-resource.openai.azure.com/"
AZURE_DEPLOYMENT_AZIENDALE = "gpt-6-luna"


def impostazione(*nomi, predefinito=""):
    """Il primo valore trovato fra i nomi dati: prima nelle variabili
    d'ambiente, poi nei Secrets di Streamlit.

    Streamlit Cloud espone i secrets di primo livello anche come variabili
    d'ambiente, ma leggerli direttamente da `st.secrets` non costa niente e
    non dipende da quel comportamento. In locale, senza un file di secrets,
    `st.secrets` solleva un'eccezione: la si ignora e si usa il predefinito."""
    for nome in nomi:
    ...
    try:
        for nome in nomi:
            if nome in st.secrets and str(st.secrets[nome]).strip():
...
if provider == "Microsoft Azure OpenAI":
    api_key = st.sidebar.text_input("API key", type="password",
                                    value=impostazione("AZURE_OPENAI_API_KEY"))
    azure_endpoint = st.sidebar.text_input(
        "Endpoint", value=impostazione("AZURE_OPENAI_ENDPOINT",
                                       predefinito=AZURE_ENDPOINT_AZIENDALE),
        placeholder="https://your-resource.openai.azure.com/",
        help="The resource address, as in the snippet you were given. With or without "
             "/openai/v1 at the end: the app adds the path itself.")
    # I deployment stanno QUI e non fra le impostazioni da esperto: su Azure
    # il nome che si chiama è quello dato da chi ha creato la risorsa, e con
    # una chiave aziendale l'elenco di solito non si può leggere. Senza nomi
    # non si parte, quindi è un dato di connessione come l'endpoint.
    # `AZURE_DEPLOYMENT` è il nome di variabile che usa lo snippet aziendale.
    azure_deployments = st.sidebar.text_input(
        "Deployments", value=impostazione("AZURE_OPENAI_DEPLOYMENT", "AZURE_DEPLOYMENT",
                                          predefinito=AZURE_DEPLOYMENT_AZIENDALE),
        placeholder="gpt-6-luna, gpt-5.6-luna",
        help="The model to use on this resource. gpt-6-luna is the company default. To try "
             "others, add them separated by commas, in order: the first answers, the others "
             "are the fallback — for example «gpt-6-luna, gpt-5.6-luna».")
    _predefinito = azure_deployments.split(",")[0].strip()
elif provider == "Anthropic Claude":
    api_key = st.sidebar.text_input("API key", type="password",
                                    value=impostazione("ANTHROPIC_API_KEY"))
    _predefinito = impostazione("ANTHROPIC_MODEL")
else:
    api_key = st.sidebar.text_input("API key", type="password",
                                    value=impostazione("GEMINI_API_KEY"))
    _predefinito = impostazione("GEMINI_MODEL")
...
if st.sidebar.button("Check the connection", disabled=not api_key, **ui.LARGA,
    ...
    diario = []
    # Su Azure si provano i deployment scritti sopra, non dei nomi generici.
    catena = build_chain(provider, api_key, azure_endpoint, azure_deployments, "qualita", diario)
    t0 = time.time()
    try:
...
with st.sidebar.expander("Expert settings"):
    ...
    if provider == "Microsoft Azure OpenAI" and (azure_deployments.strip() or _da_rete or not _modelli):
        # Su Azure la catena è quella scritta al passo 1. Se non c'è niente
        # di scritto e l'elenco non si è potuto leggere, lo si dice qui: senza
        # un nome di deployment non si analizza niente.
        model_name = azure_deployments.strip()
        if model_name:
            st.caption("Azure deployments, in order: " + " → ".join(
                d.strip() for d in model_name.split(",") if d.strip())
                + ". Change them under step 1.")
        else:
            st.warning("The deployments on this resource could not be listed. Type at least "
                       "one deployment name under step 1 — for example gpt-6-luna.")
    else:
    ...
    # Tre gradini, non quattro: Fast è «low», Balanced «medium», Deep «high».
    # Il livello più basso di tutti («minimal», che su Azure è «none») resta
    # solo per la prova di contatto, dove serve una risposta da due parole.
    RAGIONAMENTO = {"Fast": "low", "Balanced": "medium", "Deep": "high"}
