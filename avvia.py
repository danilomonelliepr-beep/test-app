...
── PERCHÉ NON C'È PIÙ `.streamlit/config.toml` ───────────────────────────
Una cartella che comincia col punto è invisibile nel gestore file e sparisce se
il progetto viene copiato trascinandolo: l'applicazione partiva senza, con
l'accento rosso di serie al posto del nostro. Le stesse impostazioni si passano
qui come variabili d'ambiente, che sono nel codice e non si perdono per strada.
I colori, comunque, sono già nel CSS di `ui.py`: anche chi lancia
`streamlit run app.py` a mano vede l'applicazione giusta.
...
# Le impostazioni che stavano nel file di configurazione, passate come opzioni
# della riga di comando. Le variabili d'ambiente equivalenti esistono, ma
