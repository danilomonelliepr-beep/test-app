...
INTERESSANTI = [
    ...
    ("connessione", re.compile(r"(Data Source=|Provider=|Server=|Driver=\{|jdbc:|"
                               r"Initial Catalog=|User Id=|DSN=)", re.I)),
    ("indirizzo", re.compile(r"https?://|ftp://|\\\\[A-Za-z0-9_.$-]+\\")),
    ("percorso", re.compile(r"[A-Za-z]:\\[^\s\"<>|]{4,}")),
    ("file", re.compile(r"\b[\w.-]{2,60}\.(?:csv|txt|xml|json|ini|cfg|dat|xls[xm]?|dbf|log|rpt)\b", re.I)),
    ("credenziale", re.compile(r"\b(password|passwd|pwd|user(?:name)?|api[_-]?key|token|secret)\b\s*[=:]", re.I)),
    ("messaggio", re.compile(r"^[A-Z][A-Za-z].{14,}[.!?]$")),
]
