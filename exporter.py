...
def prepara(analysis_result: Dict[str, Any], metadata: Dict[str, Any],
    ...
    # CORRETTO: prima si contavano solo i rischi tecnici. `impact_analysis` ha
    # anch'essa un campo `severity`, e una riga CRITICAL lì non veniva contata:
    # la copertina diceva «0» mentre tre pagine dopo il documento ne mostrava
    # una. L'indicatore a schermo (quality_indicators, in app.py) già scorre
    # tutte le sezioni — qui si allinea allo stesso conteggio, sullo stesso
    # criterio: qualunque riga con un campo `severity` valorizzato a CRITICAL
    # o HIGH, in qualunque sezione lo abbia.
    gravi = [x for nome in contract.CAMPI for x in (r.get(nome) or [])
    ...
    # Se fra i file c'è roba che non è sorgente scritto da una persona, chi
    # riceve il documento deve saperlo dalla prima pagina, non scoprirlo a
    # pagina otto leggendo un nome che non esiste nel programma.
    _prov = m.get("provenienza_file") or {}
    _decomp = [f for f, p_ in _prov.items() if p_ == "decompilato"]
    _esami = [f for f, p_ in _prov.items() if p_ == "esame"]
    if _decomp:
        copertina.append(("Decompiled files (names are the tool's, not the program's)",
                          f"{len(_decomp)}: " + ", ".join(_decomp[:4])
                          + (" \u2026" if len(_decomp) > 4 else "")))
    if _esami:
        copertina.append(("Binary triage reports (no source available)",
                          f"{len(_esami)}: " + ", ".join(_esami[:4])
                          + (" \u2026" if len(_esami) > 4 else "")))
...
def generate_pdf_report(analysis_result: Dict[str, Any], metadata: Dict[str, Any],
    ...
    for blocco in doc_dati["blocchi"]:
        ...
        if tipo == "titolo":
            ...
            if dati:
                # Le proporzioni si prendono dall'immagine (PNG o JPEG che
                # sia): una misura fissa schiaccerebbe i diagrammi alti in una
                # striscia illeggibile. Se l'intestazione non si legge, le
                # chiede ReportLab stesso all'immagine — mai un numero fisso.
                misura = mermaid_render.misura_per_riquadro(dati, larghezza, 420)
                if misura is None:
                    grezza = Image(io.BytesIO(dati))
                    fattore = min(larghezza / grezza.imageWidth, 420 / grezza.imageHeight)
                    misura = (grezza.imageWidth * fattore, grezza.imageHeight * fattore)
                l, a = misura
...
def generate_docx_report(analysis_result: Dict[str, Any], metadata: Dict[str, Any],
    ...
    for blocco in doc_dati["blocchi"]:
        ...
        if tipo == "titolo":
            ...
            dati = mermaid_render.rendi(codice, nome)
            if dati:
                misura = mermaid_render.dimensioni_immagine(dati)
