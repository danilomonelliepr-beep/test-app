...
CFG_BASE: Dict[str, Any] = {
    ...
    "attesa_max_s": 20.0,       # tetto a quanto si rispetta un Retry-After
}
...
class ErroreModello(Exception):
    ...
    def __init__(self, causa: str, dettaglio: str = "", parziale: str = "",
                 adatta: Optional[Dict[str, Any]] = None, attesa_s: float = 0.0):
        super().__init__(causa)
        self.causa = causa
        self.dettaglio = dettaglio or ""
        self.parziale = parziale or ""
        self.adatta = adatta or {}
        # Quanto il provider ha chiesto di aspettare prima di riprovare
        # (l'intestazione `Retry-After`). Zero = non l'ha detto.
        self.attesa_s = float(attesa_s or 0.0)
...
class AzureProvider(Provider):
    """Azure OpenAI, in due dialetti.

    · **v1 / Responses API** (`{endpoint}/openai/v1/responses`) — quello dello
      snippet aziendale, e quello di serie. Niente `api-version`, i parametri
      sono quelli della Responses di OpenAI: `instructions`, `input`,
      `reasoning.effort`, `max_output_tokens`, `text.format`.
    · **classico** (`/openai/deployments/{d}/chat/completions?api-version=…`)
      — per le risorse su cui il v1 non è attivo. Non si sceglie a mano: se
      il v1 risponde 404 sul percorso (e non sul deployment), per quel
      modello si passa al classico e lo si ricorda.

    Trasporto `requests` diretto, come gli altri provider, e non l'SDK
    `openai` che usa lo snippet: l'SDK riprova i 429 per conto suo, SOPRA la
    nostra catena, e si mangia il tetto di tempo; e nasconde il codice HTTP
    che la tassonomia usa per decidere. Il contenuto della richiesta è lo
    stesso dello snippet, cambia solo chi la spedisce."""

    nome = "Microsoft Azure OpenAI"
    catena_rete = ["gpt-4o", "gpt-4o-mini"]
    # Le fasce dal nome del modello. Le generazioni nuove in testa: un
    # «gpt-6-luna» non contiene «gpt-5», e prima finiva in fondo alla lista
    # dietro a un gpt-4o qualunque.
    fasce_qualita = ["gpt-9", "gpt-8", "gpt-7", "gpt-6", "o3", "gpt-5",
                     "gpt-4.1", "gpt-4o", "gpt-4", "gpt-35", "mini"]
    fasce_velocita = ["nano", "mini", "gpt-4o", "gpt-4.1", "gpt-4"]

    # I modelli che non servono a leggere codice: si tolgono dall'elenco.
    ESCLUSI = re.compile(r"embedding|whisper|tts|dall-e|sora|image|realtime|audio|transcrib",
                         re.I)
    # Le versioni dell'API con cui si prova a LEGGERE l'elenco dei deployment:
    # le recenti l'hanno tolto dal piano dati, quelle vecchie lo hanno ancora
    # su molte risorse. Se nessuna risponde, i nomi si scrivono a mano.
    VERSIONI_ELENCO = ("2023-03-15-preview", "2022-12-01")

    # Il nostro livello di ragionamento → quello della Responses API
    # (none | low | medium | high | xhigh | max). «Fast» è «none», cioè nessun
    # ragionamento: se un modello lo rifiuta si sale di un gradino da soli.
    SFORZO = {"minimal": "none", "low": "low", "medium": "medium", "high": "high"}
    # I token di ragionamento si contano DENTRO `max_output_tokens`: senza un
    # margine, a sforzo alto il modello finisce lo spazio pensando e restituisce
    # una risposta vuota o tagliata. Il margine si aggiunge al tetto richiesto.
    MARGINE_RAGIONAMENTO = {"none": 0, "low": 4000, "medium": 8000, "high": 16000}
    ...
    def __init__(self, chiave: str, endpoint: str = "", api_version: str = "",
        ...
        # ha creato la risorsa: non si ricava dal nome del modello. Se ne
        # possono scrivere più d'uno, separati da virgola: diventano la catena,
        # nell'ordine scritto.
        self.deployments = [d.strip() for d in re.split(r"[,;\n]", deployment or "") if d.strip()]
        self.deployment = self.deployments[0] if self.deployments else ""
        # Chi incolla l'endpoint dallo snippet lo incolla con /openai/v1: il
        # percorso lo mettiamo noi.
        self.endpoint = re.sub(r"/openai(/v1)?/?$", "", self.endpoint)
    ...
    def elenco(self, tetto_s: float) -> List[str]:
        ...
        ultimo = "nessuna versione dell'API espone l'elenco dei deployment"
        for versione in self.VERSIONI_ELENCO:
            url = f"{self.endpoint}/openai/deployments?api-version={versione}"
            try:
                r = con_scadenza(lambda: requests.get(url, headers=self._testate(),
                                                      timeout=(5, tetto_s)), tetto_s + 2)
            except requests.exceptions.RequestException as e:
                raise self._rete(e)
            if r.status_code in (401, 403):
                raise ErroreModello("badkey", r.text[:300])
            if not r.ok:
                ultimo = f"HTTP {r.status_code} con api-version {versione}"
                continue
            nomi = []
            for d in (r.json() or {}).get("data") or []:
                ...
                if not nome or stato not in ("succeeded", "updating", ""):
                    continue
                if self.ESCLUSI.search(nome + " " + sotto):
                    continue
                nomi.append(nome + ("\u0000" + sotto if sotto else ""))
            if nomi:
                return nomi
            ultimo = "nessun deployment utilizzabile"
        raise ErroreModello("modello", ultimo)
    ...
    def ordina(self, modelli: List[str], preferenza: str) -> List[str]:
        ...
        if self.deployments:
            ordinati = self.deployments + [n for n in ordinati if n not in self.deployments]
        return ordinati
    ...
    def chiama(self, modello: str, prompt: str, **o: Any) -> Dict[str, Any]:
        if not self.endpoint:
            raise ErroreModello("endpoint", "endpoint Azure mancante")
        adatta = dict(o.get("adatta") or {})
        if adatta.get("classica"):
            return self._chiama_classica(modello, prompt, **o)
        return self._chiama_v1(modello, prompt, **o)

    def _chiama_v1(self, modello: str, prompt: str, **o: Any) -> Dict[str, Any]:
        tetto = float(o.get("tetto_s", CFG_BASE["vera_s"]))
        adatta = dict(o.get("adatta") or {})
        voci: List[Dict[str, str]] = [{"role": "user", "content": prompt}]
        if o.get("parziale"):
            # Continuazione: il pezzo già scritto torna come turno
            # dell'assistente, e si chiede il seguito. È la forma «a stato sul
            # client» dello snippet; quella con `previous_response_id` non si
            # usa perché vorrebbe `store: true` (vedi sotto).
            voci.append({"role": "assistant", "content": o["parziale"]})
            voci.append({"role": "user", "content": _CONTINUA})
        corpo: Dict[str, Any] = {
            "model": modello,
            "input": voci,
            # Di serie la Responses API CONSERVA la risposta sul server, per
            # poterla riprendere con `previous_response_id`. A noi non serve,
            # e il prompt contiene il sorgente del cliente: non deve restare
            # su una risorsa condivisa oltre la chiamata.
            "store": False,
        }
        if o.get("sistema"):
            corpo["instructions"] = o["sistema"]

        tetto_token = int(o.get("max_token", CFG_BASE["max_token"]))
        if not adatta.get("no_reasoning"):
            sforzo = self.SFORZO[_almeno(o.get("ragionamento") or "low", adatta.get("thinking_min"))]
            corpo["reasoning"] = {"effort": sforzo}
            tetto_token += self.MARGINE_RAGIONAMENTO.get(sforzo, 0)
        elif not adatta.get("no_temperature"):
            # `temperature` solo ai modelli che NON ragionano: quelli che
            # ragionano la rifiutano.
            corpo["temperature"] = 0
        corpo["max_output_tokens"] = tetto_token
        ...
        try:
            r = con_scadenza(lambda: requests.post(url, headers=self._testate(), json=corpo,
                                                   timeout=(5, tetto)), tetto)
        except requests.exceptions.RequestException as e:
            raise self._rete(e)
        self._stato(r, {**o, "_v1": True})
        j = r.json() or {}

        testo, rifiutato = [], False
        for voce in j.get("output") or []:
            if voce.get("type") != "message":
                continue          # i «reasoning» sono il pensiero, non la risposta
            for pezzo in voce.get("content") or []:
                if pezzo.get("type") == "output_text":
                    testo.append(pezzo.get("text") or "")
                elif pezzo.get("type") == "refusal":
                    rifiutato = True
        testo = "".join(testo)
        motivo = str((j.get("incomplete_details") or {}).get("reason") or "")
        if rifiutato or motivo == "content_filter":
            raise ErroreModello("blocked", motivo or "refusal")
        if j.get("status") == "incomplete" and motivo == "max_output_tokens":
            raise ErroreModello("troncata", motivo, parziale=testo)
        if j.get("status") == "failed":
            raise ErroreModello("busy", str(j.get("error") or "failed"))
        if not testo.strip():
            raise ErroreModello("vuota", motivo or str(j.get("status") or ""))
        return {"testo": testo, "uso": j.get("usage") or {}}

    def _chiama_classica(self, modello: str, prompt: str, **o: Any) -> Dict[str, Any]:
        tetto = float(o.get("tetto_s", CFG_BASE["vera_s"]))
        adatta = dict(o.get("adatta") or {})
        ...
        if (re.search(r"\b(o\d|gpt-(?:[5-9]|\d{2})|reason)", modello, re.I)
    ...
    # ── Come si leggono i guasti ────────────────────────────────────────
    @staticmethod
    def _secondi(r) -> float:
        try:
            return float((getattr(r, "headers", None) or {}).get("Retry-After") or 0)
    ...
    def _stato(self, r, o: Optional[Dict[str, Any]] = None) -> None:
        ...
        det = r.text[:600]
        if r.status_code == 429:
            # Su Azure un 429 è quasi sempre il limite di token o richieste al
            # minuto DEL DEPLOYMENT: passa da solo, e il provider dice quanto
            # aspettare. Prima valeva «quota», che fa cambiare modello — e con
            # più lotti in parallelo lotti diversi finivano scritti da modelli
            # diversi. Ora si aspetta e si riprova lo stesso; «quota» resta
            # solo quando il messaggio dice che il credito è finito davvero.
            if re.search(r"insufficient_quota|quota.*exceeded|billing|exhausted", det, re.I):
                raise ErroreModello("quota", det)
            raise ErroreModello("busy", det, attesa_s=self._secondi(r) or 5.0)
        if r.status_code == 404:
            if o.get("_v1") and not re.search(r"DeploymentNotFound|deployment|does not exist|model_not_found",
                                              det, re.I):
                # Il percorso /openai/v1 non c'è su questa risorsa: per questo
                # modello si passa al dialetto classico, e lo si ricorda.
                raise ErroreModello("parametro", det, adatta={"classica": True})
            raise ErroreModello("modello", det)
        if r.status_code in (500, 502, 503, 504):
            raise ErroreModello("busy", det, attesa_s=self._secondi(r))
        if r.status_code in (401, 403):
            raise ErroreModello("badkey", det)
        if r.status_code == 400:
            ...
            if re.search(r"context length|maximum context|too many tokens|reduce the length", det, re.I):
                raise ErroreModello("contesto", det)
            if re.search(r"content_filter|content management policy|ResponsibleAI", det, re.I):
                raise ErroreModello("blocked", det)
            ...
            if re.search(r"reasoning_effort|reasoning", det, re.I):
                ...
                raise ErroreModello("parametro", det, adatta=patch)
            if re.search(r"response_format|json_object|json_schema|text\.format", det, re.I):
...
class CatenaModelli:
    def __init__(self, provider: str, chiave: str, endpoint: str = "", api_version: str = "",
        ...
        # Su Azure `deployment` può essere un ELENCO («gpt-6-luna, gpt-5.6-luna»):
        # il preferito è il primo nome, non la stringa intera con le virgole —
        # che la catena chiamava letteralmente come se fosse un deployment.
        self.preferito = re.split(r"[,;\n]", (preferito or deployment or ""))[0].strip()
    ...
    def catena(self, forza: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Su Azure i nomi scritti a mano SONO la catena: l'elenco dei
            # deployment spesso non si può leggere con una chiave, e i nomi di
            # riserva generici (gpt-4o…) su una risorsa aziendale non esistono.
            scritti = list(getattr(self.p, "deployments", []) or [])
            if scritti:
                rete = scritti + [n for n in rete if n not in scritti]
            ...
            while tentativo < int(self.C["tentativi_vera"]):
                ...
                try:
                    ...
                    if e.causa in RIPROVA:
                        tentativo += 1
                        time.sleep(self._attesa(e, tentativo))
                        continue
                    break
                except Exception as e:  # noqa: BLE001
                    ultima = "rete"
    ...
    def _attesa(self, e: "ErroreModello", tentativo: int) -> float:
        """Quanto aspettare prima di riprovare lo stesso modello: la pausa che
        cresce a ogni tentativo, oppure quella che il provider ha chiesto —
        la più lunga delle due, con un tetto. Riprovare prima di quando il
        provider ha detto è solo un modo di farsi dire di nuovo di no."""
        chiesta = min(e.attesa_s, float(self.C["attesa_max_s"]))
        return max(self.C["pausa_base_s"] * tentativo, chiesta)
