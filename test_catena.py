...
# =============================================================================
class FintaRisposta:
    def __init__(self, status, corpo=None, testo="", testate=None):
...
# =============================================================================
# IL TEMA DA CODICE, SENZA CARTELLA NASCOSTA
# (in un processo a parte: qui dentro `streamlit` verrà sostituito da un finto,
#  e lo Streamlit vero non sopporta di convivere con un finto nello stesso
#  processo. Un processo pulito è l'unico modo onesto di provarlo.)
# =============================================================================
import subprocess as _sp

_prova_tema = _sp.run([sys.executable, "-c", """
import sys
try:
    import streamlit.config as c, ui
except Exception as e:
    print("SALTA", e); sys.exit(0)
for k, v in ui.TEMA_STREAMLIT.items():
    c.set_option(k, v)
ok = (c.get_option("theme.primaryColor") == "#15616D" and c.get_option("server.maxUploadSize") == 50
      and ui.TEMA_STREAMLIT["theme.primaryColor"].lower() in ui.TEMA.lower())
print("OK" if ok else "KO")
"""], capture_output=True, text=True, cwd=str(QUI_TEST))
_esito_tema = (_prova_tema.stdout or "").strip().splitlines()[-1:] or [""]
if _esito_tema[0].startswith("SALTA"):
    print("· il tema da codice: saltato (Streamlit non installato)")
else:
    P("il tema di Streamlit si imposta da codice, opzioni «non modificabili» comprese, "
      "con gli stessi colori del CSS", _esito_tema[0] == "OK")
P("nel pacchetto non c'è nessuna cartella nascosta",
  not any(p_.name.startswith(".") and p_.is_dir() for p_ in QUI_TEST.iterdir()))
...
P("un'immagine illeggibile non produce MAI una misura fissa: torna None e decide chi chiama",
  mr.misura_per_riquadro(b"non-e-un-png", 780, 460) is None)


def _jpeg(w, h):
    """Un JPEG minimo: l'intestazione con il segmento SOF, come li manda mermaid.ink."""
    import struct
...
P("le dimensioni si leggono anche da un JPEG (è quello che manda mermaid.ink di serie)",
  mr.dimensioni_immagine(_jpeg(800, 2000)) == (800, 2000))
l, a = mr.misura_per_riquadro(_jpeg(800, 2000), 780, 460)
P(f"un diagramma alto arrivato come JPEG non viene schiacciato ({l:.0f}x{a:.0f} pt)",
  abs(a / l - 2.5) < 0.01)
P("al servizio esterno si chiede esplicitamente un PNG",
  "?type=png" in open(QUI_TEST / "mermaid_render.py").read())
...
# Un eseguibile non viene più respinto: viene esaminato (vedi in fondo). Quello
# che resta respinto è un binario che eseguibile non è.
_dll = app.build_source_collection([_Finto("x.dll", b"MZ\x90\x00\x03binario" + b"\x00" * 40)], "", "")
P("una DLL non viene respinta: viene esaminata",
  _dll[0]["provenienza"] == "esame" and "BINARY TRIAGE" in _dll[0]["content"])
try:
    app.build_source_collection([_Finto("x.dat", b"\x01\x02\x00binario qualunque" + b"\x00" * 40)], "", "")
    ESITI.append(False); print("✗ un binario non eseguibile doveva essere respinto")
except ValueError as e:
    P("un binario che non è un eseguibile resta respinto, con il rimedio", "binary" in str(e))
...
# =============================================================================
# IL DIARIO CHE SI LEGGE, E L'ANALISI SALVATA RICONOSCIUTA DA SOLA
# =============================================================================
AI, _ = nuova({"*": {"stato": 200}})
AI.chiedi("x")
diario_testo = "\n".join(AI.diario)
P("il diario è in inglese, come il resto dell'interfaccia",
  "chain (" in diario_testo and "probing:" in diario_testo
  and not any(p in diario_testo for p in ("Catena", "Prova di contatto", "risponde")))
P("la catena esce un modello per riga, non su una riga sola che viene tagliata",
  diario_testo.count("\n  1. ") == 1 and "  2. " in diario_testo and " → " not in diario_testo)

_analisi = _json.dumps({"contract_version": "2.1", "executive_summary": "S.",
                        "application_purpose": "P.",
                        "business_rules": [{"rule_name": "R", "condition": "c",
                                            "sme_approved": True}]}).encode()
ric_json, _ = app.carica_analisi_salvata(_analisi)
P("un'analisi esportata si riconosce dal contenuto e conserva le spunte",
  ric_json["business_rules"][0]["sme_approved"] is True)
for _estraneo in (b'{"timeout": 30, "retries": 3}', b'{"models": ["a"]}', b"non json"):
    try:
        app.carica_analisi_salvata(_estraneo)
        ESITI.append(False)
        print("✗ un JSON estraneo doveva restare un file da analizzare")
        break
    except (ValueError, Exception):
        continue
else:
    ESITI.append(True)
    print("✓ un JSON qualunque resta un file da analizzare, non una ripresa")



# =============================================================================
# QUANDO IL SORGENTE NON C'È: ESEGUIBILI E CODICE DECOMPILATO
# =============================================================================
import binary_triage as _bt
...
_ghidra = ("void FUN_00401a30(int iVar1,undefined4 *puVar2)\n{\n"
           "  undefined4 uVar3;\n  uVar3 = *(undefined4 *)(DAT_00420a18 + 8);\n"
           "  if (iVar1 < 100) { sub_401200(uVar3); }\n"
           "  /* WARNING: Subroutine does not return */\n}\n")
_exe_net = (b"MZ" + b"\x00" * 0x3a + b"\x80\x00\x00\x00" + b"\x00" * 0x40 + b"PE\x00\x00"
            + b"\x4c\x01\x03\x00" + b"\x00" * 220
            + b"mscoree.dll\x00BSJB\x00SELECT * FROM ORDINI\x00OCI.dll\x00")

P("un .NET si riconosce, e si dice che il sorgente è quasi recuperabile",
  any(t["codice"] == "NET" for t in _bt.esamina(_exe_net, "X.EXE")["tecnologie"]))
P("un binario Linux qualunque non viene scambiato per Delphi",
  not any(t["codice"] == "DELPHI" for t in _bt.esamina(open("/usr/bin/objdump", "rb").read()
                                                        if _P("/usr/bin/objdump").exists()
                                                        else b"\x7fELF" + b"Borland" * 3,
                                                        "objdump")["tecnologie"]))
_esito_bin = _bt.esamina(_exe_net, "X.EXE")
P("le librerie che raccontano qualcosa vengono spiegate",
  any("Oracle" in (l["cosa_dice"] or "") for l in _esito_bin["librerie"]))
P("le stringhe utili vengono tirate fuori per categoria",
  "SQL" in _esito_bin["stringhe"])
P("il resoconto è testo, quindi l'estrattore lo sa leggere",
  "BINARY TRIAGE REPORT" in _bt.resoconto(_esito_bin))

P("il codice decompilato si riconosce da solo", app.riconosci_provenienza(_ghidra) == "decompilato")
P("il sorgente vero non viene scambiato per decompilato",
  app.riconosci_provenienza("PROCEDURE CALC IS BEGIN sub_totale(1); END;") == "sorgente")

_src = app.build_source_collection(
    [_FF("dump.c", _ghidra.encode()), _FF("p.pkb", b"PROCEDURE C IS BEGIN NULL; END;"),
     _FF("OLD.EXE", _exe_net)], "", "")
_prov = {s_["filename"]: s_["provenienza"] for s_ in _src}
P(f"un eseguibile non viene più respinto: diventa il resoconto del suo esame ({_prov})",
  _prov == {"dump.c": "decompilato", "p.pkb": "sorgente", "OLD.EXE": "esame"})
try:
    app.build_source_collection([_FF("x.fmb", b"\x00\x01binario qualunque" + b"\x00" * 50)], "", "")
    ESITI.append(False); print("✗ un binario non eseguibile doveva essere respinto")
except ValueError:
    ESITI.append(True); print("✓ un binario non eseguibile resta respinto, con il rimedio")

_nota = app.nota_provenienza(_src)
P("il prompt avverte il modello, nominando i file",
  "DECOMPILED" in _nota and "dump.c" in _nota and "BINARY TRIAGE" in _nota
  and "OLD.EXE" in _nota)
P("la nota entra nel prompt vero, attraverso il gancio che il contratto ha già",
  "DECOMPILED" in contract.prompt_analisi(_src, {}, (1, 1), note=_nota))

_meta_bin = app.extract_technical_metadata(_src)
_ris_bin = app.merge_static_and_ai_results(contract.normalizza({"business_rules": [
    {"rule_name": "Da dump", "condition": "x>100", "source_file": "dump.c",
     "confidence": "HIGH", "evidence": "riga 6"},
    {"rule_name": "Da sorgente", "condition": "x>100", "source_file": "p.pkb",
     "confidence": "HIGH", "evidence": "riga 1"}]}), _meta_bin)
_per_file = {x["source_file"]: x for x in _ris_bin["business_rules"]}
P("una riga che nasce da codice decompilato non può dichiararsi certa",
  _per_file["dump.c"]["confidence"] == "MEDIUM"
  and "from decompiled code" in _per_file["dump.c"]["evidence"])
P("una riga che nasce da sorgente vero resta com'è",
  _per_file["p.pkb"]["confidence"] == "HIGH"
  and "decompiled" not in _per_file["p.pkb"]["evidence"])
P("il documento lo dice in copertina",
  any("Decompiled" in k for k, _ in exporter.prepara(_ris_bin, _meta_bin, "g", "m")["copertina"]))
P("senza file decompilati la copertina non cambia",
  not any("Decompiled" in k for k, _ in exporter.prepara(
      contract.normalizza({}), {"file_count": 1, "files": []}, "g", "m")["copertina"]))


# =============================================================================
# IL RESOCONTO DI UN BINARIO NON DEVE ANALIZZARE SÉ STESSO COME CODICE
# (segnalazione dal campo: "LIBRARIES", "FUNCTIONS", "PERCORSO" comparivano
# come dipendenze finte, prese dalle intestazioni del resoconto stesso)
# =============================================================================
_exe_reale = (b"MZ" + b"\x00" * 0x3a + b"\x80\x00\x00\x00" + b"\x00" * 0x40 + b"PE\x00\x00"
             + b"\x4c\x01\x03\x00" + b"\x00" * 220
             + b"mscoree.dll\x00BSJB\x00SELECT sIntCode FROM vw_WrongClassification\x00")
_src_esame = app.build_source_collection([_FF("Tools.exe", _exe_reale)], "", "")
P("il resoconto di un binario si riconosce come tale, non come codice",
  _src_esame[0]["provenienza"] == "esame")
_meta_esame = app.extract_technical_metadata(_src_esame)
P("le intestazioni del proprio resoconto («LINKED LIBRARIES (», «STRINGS — PATH (») "
  "non diventano dipendenze finte",
  not any(d["target"] in ("LIBRARIES", "FUNCTIONS", "PERCORSO", "PATH", "SQL")
          for d in _meta_esame["dependencies"]))
P("nessuna dipendenza inventata esce affatto dal resoconto di un binario",
  _meta_esame["dependencies"] == [])
_esito_reale = _bt.esamina(_exe_reale, "Tools.exe")
P("la regex generica, isolata, avrebbe preso proprio quella parola dal resoconto "
  "(prova diretta della causa: l'intestazione «LINKED LIBRARIES (» diventa «LIBRARIES»)",
  "LIBRARIES" in set(re.findall(r"\b([A-Za-z_][A-Za-z0-9_$.]*)\s*\(",
                                _bt.resoconto(_esito_reale))))
P("le categorie delle stringhe sono in inglese, come il resto del documento",
  set(c for c, _ in _bt.INTERESSANTI) == {"SQL", "connection string", "address",
                                          "path", "file", "credential", "message"})


# =============================================================================
# TRE FALLE NEL MECCANISMO DI FIDUCIA, TROVATE RILEGGENDO UN PDF VERO
# =============================================================================

# ── 1. il tetto di confidenza deve valere per OGNI sezione, non solo due nomi ──
_r_tetto = contract.normalizza({
    "data_flows": [{"source": "vw_View", "target": "Tools.exe",
                    "data_description": "x", "confidence": "HIGH"}],
    "application_mapping": [{"source_component": "Tools.exe", "external_system": "DB",
                             "confidence": "HIGH"}],
    "impact_analysis": [{"change_scenario": "x", "affected_components": ["Tools.exe"],
                         "severity": "CRITICAL", "confidence": "HIGH"}],
})
_r_tetto = app.abbassa_confidenza_non_sorgente(_r_tetto, {"Tools.exe": "esame"})
P("il tetto di confidenza raggiunge data_flows (campi source/target, non source_file)",
  _r_tetto["data_flows"][0]["confidence"] == "MEDIUM")
P("il tetto raggiunge application_mapping (source_component, non source_file)",
  _r_tetto["application_mapping"][0]["confidence"] == "MEDIUM")
P("il tetto raggiunge impact_analysis (affected_components AL PLURALE, non affected_component)",
  _r_tetto["impact_analysis"][0]["confidence"] == "MEDIUM")

# ── 2. il modello non può dichiararsi «parser» da solo ─────────────────────
_grezzo_spoof = app.forza_provenienza_modello(
    {"components": [{"component_name": "Y", "source": "STATIC_ANALYSIS", "confidence": "HIGH"}],
     "technical_risks": [{"risk_type": "X", "source": "STATIC_ANALYSIS", "severity": "LOW"}]})
_r_spoof = contract.normalizza(_grezzo_spoof)
P("la risposta grezza del modello non può auto-dichiararsi «found by parser»",
  _r_spoof["components"][0]["source"] == "LLM_ANALYSIS"
  and _r_spoof["technical_risks"][0]["source"] == "LLM_ANALYSIS")

_md_vero = {"components": [{"component_name": "Z", "component_type": "PROCEDURE",
                            "source_file": "p.pkb", "confidence": "HIGH",
                            "source": "STATIC_ANALYSIS"}],
           "dependencies": [], "interfaces": [], "data_objects": [], "local_risks": [], "files": []}
_fuso = app.merge_static_and_ai_results(contract.normalizza({}), _md_vero)
P("una riga che viene DAVVERO dal parser (fusa dopo) resta etichettata «parser»",
  _fuso["components"] and _fuso["components"][0]["source"] == "STATIC_ANALYSIS")

# ── 3. la copertina conta i CRITICAL/HIGH di ogni sezione, non solo i rischi tecnici ──
_prep_grave = exporter.prepara(
    _r_tetto, {"file_count": 1, "total_line_count": 1, "languages": [], "detected_tables": [],
              "components": [], "dependencies": [], "interfaces": [], "data_objects": [],
              "local_risks": [], "files": [], "provenienza_file": {"Tools.exe": "esame"}}, "g", "m")
P("la copertina conta anche un CRITICAL trovato in impact_analysis, non solo in technical_risks",
  dict(_prep_grave["copertina"])["Critical and high risks"] == "1")
P("una copertina senza rischi gravi da nessuna parte resta a 0",
  dict(exporter.prepara(contract.normalizza({}),
      {"file_count": 1, "files": []}, "g", "m")["copertina"])["Critical and high risks"] == "0")


# =============================================================================
# UN ESEGUIBILE VERO PESA PIÙ DI 2 MB, E NON DEVE ESSERE RESPINTO PER QUESTO
# (segnalazione: "PrIDE.dll exceeds the allowed size")
# =============================================================================
def _pe_di(peso):
    return (b"MZ" + b"\x00" * 0x3a + b"\x80\x00\x00\x00" + b"\x00" * 0x40 + b"PE\x00\x00"
            + b"\x4c\x01\x03\x00" + b"\x00" * 220 + b"mscoree.dll\x00" + b"\x00" * peso)


_dll_grossa = _pe_di(8 * 1024 * 1024)   # 8 MB: sopra il tetto del testo, comune per una DLL vera
_src_dll = app.build_source_collection([_FF("PrIDE.dll", _dll_grossa)], "", "")
P("una .dll più pesante del tetto per il testo sorgente (8 MB > 2 MB) viene esaminata, non respinta",
  _src_dll[0]["provenienza"] == "esame")

try:
    app.build_source_collection([_FF("Mostro.dll", _pe_di(55 * 1024 * 1024))], "", "")
    ESITI.append(False); print("✗ un eseguibile sopra i 50 MB doveva essere respinto")
except ValueError as e:
    P("resta un tetto, più alto: un eseguibile sopra i 50 MB viene respinto col numero giusto",
      "50 MB" in str(e))

try:
    app.build_source_collection([_FF("enorme.sql", b"SELECT 1;\n" * 300000)], "", "")
    ESITI.append(False); print("✗ un sorgente di testo sopra i 2 MB doveva restare respinto")
except ValueError as e:
    P("il tetto sul testo sorgente resta quello di prima: non è cambiato per il codice vero",
      "MB" not in str(e))   # il messaggio per il testo non cita un numero, per non doverlo tenere sincronizzato in due posti


# =============================================================================
# IL NOME DEL PRODOTTO SI LEGGE ANCHE COME SIGLA, LAKE
# =============================================================================
import ui as _ui
P("il banner sta dentro il modulo, non in un file che si può perdere per strada",
  _ui.BANNER_SVG.startswith("<svg") and not (QUI_TEST / "assets").exists())
P("il banner è leggero: pochi KB, perché viaggia dentro la pagina",
  len(_ui.BANNER_SVG) < 20_000)
P("il banner usa i colori esatti del tema, non colori suoi",
  "#15616D" in _ui.BANNER_SVG and "#0E434C" in _ui.BANNER_SVG)
_html_testata = []
_vero_markdown = _ui.st.markdown
_ui.st.markdown = lambda t, **k: _html_testata.append(str(t))
_ui.testata("LAKE: Legacy Application Knowledge Extractor", "compito")
_ui.st.markdown = _vero_markdown
P("il titolo resta scritto nella pagina per chi la ascolta invece di guardarla",
  'class="solo-lettori"' in _html_testata[0]
  and "LAKE: Legacy Application Knowledge Extractor" in _html_testata[0])
P("e il banner è nella stessa testata, inline",
  "<svg" in _html_testata[0])

_titolo = _ui._con_iniziali_in_evidenza("LAKE: Legacy Application Knowledge Extractor")
P("la sigla davanti ai due punti è evidenziata per intero",
  _titolo.startswith('<span class="sigla">LAKE</span>:'))
P("e anche le iniziali delle parole che la compongono, così il legame si vede",
  [c for c in "LAKE"] == [_titolo.split('class="sigla">')[i].split("<")[0]
                          for i in range(2, 6)])
P("il resto di ogni parola non viene toccato",
  "egacy" in _titolo and "pplication" in _titolo and "nowledge" in _titolo
  and "xtractor" in _titolo)
P("senza i due punti si evidenziano le sole iniziali",
  _ui._con_iniziali_in_evidenza("Due Parole").count('class="sigla"') == 2)


# =============================================================================
# AZURE AZIENDALE: RESPONSES API (/openai/v1), COME NELLO SNIPPET
# Un Azure finto che risponde nel formato vero della Responses API. Il provider
# Azure non aveva MAI avuto un collaudo, nemmeno con un finto.
# =============================================================================
import model_chain as _mc

AZ_ENDPOINT = "https://2026-epr-hackathon-resource.openai.azure.com/"
_az = {"richieste": [], "copione": {}}


def _risposta_v1(testo, stato="completed", motivo=None):
    corpo = {"status": stato, "output": [
        {"type": "reasoning", "summary": []},
        {"type": "message", "role": "assistant",
         "content": [{"type": "output_text", "text": testo}]}],
        "usage": {"input_tokens": 10, "output_tokens": 5}}
    if motivo:
        corpo["incomplete_details"] = {"reason": motivo}
    return corpo


def _az_post(url, headers=None, json=None, timeout=None):
    corpo = json or {}
    _az["richieste"].append({"url": url, "corpo": corpo, "testate": headers or {}})
    modello = corpo.get("model", "")
    azione = _az["copione"].get(modello) or _az["copione"].get("*") or {"stato": 200}
    if callable(azione):
        return azione(url, corpo)
    if azione.get("stato", 200) != 200:
        return FintaRisposta(azione["stato"], {}, azione.get("testo", "errore"),
                             testate=azione.get("testate"))
    if "/openai/v1/responses" in url:
        return FintaRisposta(200, _risposta_v1(azione.get("testo", "OK")))
    return FintaRisposta(200, {"choices": [{"message": {"content": azione.get("testo", "OK")},
                                            "finish_reason": "stop"}]})


def _az_get(url, headers=None, timeout=None):
    elenco = _az["copione"].get("_elenco")
    if elenco is None:
        return FintaRisposta(404, {}, "Resource not found")
    return FintaRisposta(200, {"data": elenco})


def _azure(copione, deployment="", ragionamento="low", memoria=None):
    _az["richieste"].clear()
    _az["copione"] = copione
    _mc.requests = types.SimpleNamespace(get=_az_get, post=_az_post,
                                         utils=__import__("requests").utils,
                                         exceptions=__import__("requests").exceptions)
    return CatenaModelli(provider="azure", chiave="k-az", endpoint=AZ_ENDPOINT,
                         deployment=deployment, ragionamento=ragionamento,
                         memoria=memoria or MemoriaRam(),
                         cfg={"pausa_base_s": 0.01, "attesa_max_s": 0.05})


# ── la chiamata ha la forma dello snippet ────────────────────────────────
AZ = _azure({"*": {"testo": "Blast chillers cool food fast."}}, deployment="gpt-6-luna")
r = AZ.chiedi("What is a blast chiller?", sistema="You are terse.", json_mode=False)
_vera = [q for q in _az["richieste"] if "OK." not in json.dumps(q["corpo"])][-1]
P("la chiamata va a {endpoint}/openai/v1/responses, come nello snippet",
  _vera["url"] == "https://2026-epr-hackathon-resource.openai.azure.com/openai/v1/responses")
P("il corpo è quello della Responses API: model, instructions, input, reasoning",
  _vera["corpo"]["model"] == "gpt-6-luna"
  and _vera["corpo"]["instructions"] == "You are terse."
  and _vera["corpo"]["input"][0] == {"role": "user", "content": "What is a blast chiller?"}
  and _vera["corpo"]["reasoning"] == {"effort": "low"})
P("la risposta si legge dagli elementi `message`, non da quelli di ragionamento",
  r.testo == "Blast chillers cool food fast." and r.modello == "gpt-6-luna")
P("il sorgente del cliente non resta sul server: store=false",
  _vera["corpo"]["store"] is False)
P("i token di ragionamento hanno il loro margine dentro max_output_tokens",
  _vera["corpo"]["max_output_tokens"] > _mc.CFG_BASE["max_token"])
P("la chiave va nelle intestazioni, sia come api-key sia come Bearer",
  _vera["testate"]["api-key"] == "k-az" and _vera["testate"]["Authorization"] == "Bearer k-az")

AZ = _azure({"*": {"testo": "x"}}, deployment="gpt-6-luna")
AZ.chiedi("q", json_mode=True)
P("il modo JSON della Responses API: text.format = json_object",
  _az["richieste"][-1]["corpo"]["text"] == {"format": {"type": "json_object"}})

# ── l'endpoint incollato con /openai/v1 dallo snippet non si raddoppia ──
_p = _mc.AzureProvider("k", "https://x.openai.azure.com/openai/v1/", deployment="d")
P("l'endpoint incollato con /openai/v1/ viene ripulito, il percorso lo mettiamo noi",
  _p.endpoint == "https://x.openai.azure.com")

# ── i due deployment dello snippet, scritti a mano, sono la catena ─────
AZ = _azure({"*": {"testo": "ok"}}, deployment="gpt-6-luna, gpt-5.6-luna")
_cat = AZ.catena()
P("se l'elenco dei deployment non si legge, i nomi scritti a mano SONO la catena, "
  "nell'ordine scritto",
  _cat["lista"][:2] == ["gpt-6-luna", "gpt-5.6-luna"] and _cat["fonte"].startswith("rete"))

AZ = _azure({"gpt-6-luna": {"stato": 404, "testo": '{"error":{"code":"DeploymentNotFound"}}'},
             "*": {"testo": "risposta"}}, deployment="gpt-6-luna, gpt-5.6-luna")
r = AZ.chiedi("q")
P("se il primo deployment non esiste, si scende al secondo",
  r.modello == "gpt-5.6-luna")
P("nessuna chiamata a un deployment di nome «gpt-6-luna, gpt-5.6-luna» "
  "(la stringa con le virgole non è un nome)",
  not any("," in q["corpo"].get("model", "") for q in _az["richieste"]))

# ── la scoperta, quando funziona: niente modelli di immagini fra quelli per il testo ──
AZ = _azure({"_elenco": [
    {"id": "gpt-5.6-luna", "model": "gpt-5.6", "status": "succeeded"},
    {"id": "gpt-6-luna", "model": "gpt-6", "status": "succeeded"},
    {"id": "gpt-image-2", "model": "gpt-image-2", "status": "succeeded"},
    {"id": "emb", "model": "text-embedding-3-large", "status": "succeeded"}]})
_cat = AZ.catena(forza=True)
P(f"la scoperta dei deployment: gpt-6 prima di gpt-5.6, immagini ed embedding fuori ({_cat['lista']})",
  _cat["lista"] == ["gpt-6-luna", "gpt-5.6-luna"])

# ── il ragionamento: i nostri quattro livelli nei nomi della Responses ──
for _nostro, _loro in (("minimal", "none"), ("low", "low"), ("medium", "medium"), ("high", "high")):
    AZ = _azure({"*": {"testo": "ok"}}, deployment="gpt-6-luna", ragionamento=_nostro)
    AZ.chiedi("q")
    if _az["richieste"][-1]["corpo"]["reasoning"]["effort"] != _loro:
        ESITI.append(False); print(f"✗ {_nostro} doveva diventare {_loro}")
        break
else:
    ESITI.append(True)
    print("✓ i livelli interni minimal/low/medium/high diventano none/low/medium/high")


def _rifiuta_none(url, corpo):
    if corpo.get("reasoning", {}).get("effort") == "none":
        return FintaRisposta(400, {}, "Unsupported value: 'reasoning.effort' does not support "
                                      "'none' with this model.")
    return FintaRisposta(200, _risposta_v1("ok"))


AZ = _azure({"*": _rifiuta_none}, deployment="gpt-5.6-luna", ragionamento="minimal")
r = AZ.chiedi("q")
_sforzi = [q["corpo"].get("reasoning", {}).get("effort") for q in _az["richieste"]]
P(f"se un modello rifiuta «none», si sale da soli a «low» e si va avanti ({_sforzi})",
  r.testo == "ok" and _sforzi[-1] == "low")

# ── i 429 di Azure: si aspetta e si riprova LO STESSO modello ───────────
_conta429 = {"n": 0}


def _limite(url, corpo):
    if "OK." in json.dumps(corpo):
        return FintaRisposta(200, _risposta_v1("OK"))
    _conta429["n"] += 1
    if _conta429["n"] < 2:
        return FintaRisposta(429, {}, "Requests to the Responses API have exceeded token rate limit",
                             testate={"Retry-After": "3"})
    return FintaRisposta(200, _risposta_v1("dopo l'attesa"))


AZ = _azure({"gpt-6-luna": _limite, "*": {"testo": "SBAGLIATO: altro modello"}},
            deployment="gpt-6-luna, gpt-5.6-luna")
r = AZ.chiedi("q")
P("un 429 di Azure (limite al minuto) si aspetta e si riprova sullo stesso modello, "
  "non si passa a un altro",
  r.modello == "gpt-6-luna" and r.testo == "dopo l'attesa")
_e429 = _mc.ErroreModello("busy", attesa_s=3)
P("l'attesa chiesta dal provider viene rispettata, entro il tetto",
  AZ._attesa(_e429, 1) == AZ.C["attesa_max_s"])

AZ = _azure({"gpt-6-luna": {"stato": 429, "testo": '{"error":{"code":"insufficient_quota"}}'},
             "*": {"testo": "dal secondo"}}, deployment="gpt-6-luna, gpt-5.6-luna")
r = AZ.chiedi("q")
P("un 429 che dice che il credito è finito davvero fa passare al modello successivo",
  r.modello == "gpt-5.6-luna")

# ── risposta tagliata e continuazione ───────────────────────────────────
AZ = _azure({"*": lambda url, c: FintaRisposta(200, _risposta_v1('{"a": [1, 2', "incomplete",
                                                                 "max_output_tokens"))},
            deployment="gpt-6-luna")
r = AZ.chiedi("q", json_mode=True)
P("status=incomplete per max_output_tokens è una risposta TAGLIATA, da continuare",
  r.troncata and r.testo == '{"a": [1, 2')

AZ = _azure({"*": {"testo": ', 3]}'}}, deployment="gpt-6-luna")
s_ = AZ.continua("gpt-6-luna", "q", '{"a": [1, 2', sistema="s", ragionamento="low")
_c = _az["richieste"][-1]["corpo"]
P("la continuazione rimanda il pezzo scritto come turno dell'assistente, senza json_object",
  s_.testo == ", 3]}" and _c["input"][1] == {"role": "assistant", "content": '{"a": [1, 2'}
  and _c["input"][2]["role"] == "user" and "text" not in _c)

# ── la risorsa senza /openai/v1: si passa al dialetto classico, da soli ──
def _senza_v1(url, corpo):
    if "/openai/v1/" in url:
        return FintaRisposta(404, {}, '{"error":{"code":"404","message":"Resource not found"}}')
    return FintaRisposta(200, {"choices": [{"message": {"content": "dal classico"},
                                            "finish_reason": "stop"}]})


_mem = MemoriaRam()
AZ = _azure({"*": _senza_v1}, deployment="gpt-4o", memoria=_mem)
r = AZ.chiedi("q")
P("se la risorsa non ha /openai/v1, si passa al dialetto classico e si risponde",
  r.testo == "dal classico" and "chat/completions" in _az["richieste"][-1]["url"])
P("e il passaggio si ricorda: la volta dopo si parte già dal classico",
  _mem.leggi_adatta(AZ.p.spazio(), "gpt-4o").get("classica") is True)

# ── le risposte rifiutate dal filtro dei contenuti ─────────────────────
AZ = _azure({"gpt-6-luna": lambda u, c: FintaRisposta(200, {"status": "incomplete",
             "incomplete_details": {"reason": "content_filter"}, "output": []}),
             "*": {"testo": "dal secondo"}}, deployment="gpt-6-luna, gpt-5.6-luna")
r = AZ.chiedi("q")
P("un blocco del filtro dei contenuti fa passare al modello successivo",
  r.modello == "gpt-5.6-luna")


# =============================================================================
# L'APP SI APRE GIÀ COMPILATA, CON TRE GRADINI DI RAGIONAMENTO
# =============================================================================
P("tre gradini di ragionamento: Fast low, Balanced medium, Deep high",
  app.RAGIONAMENTO == {"Fast": "low", "Balanced": "medium", "Deep": "high"})
P("Balanced, il predefinito, è «medium»", app.ragionamento == "medium")
P("si apre su Azure, con l'endpoint aziendale già scritto",
  app.provider == "Microsoft Azure OpenAI"
  and app.azure_endpoint == "https://2026-epr-hackathon-resource.openai.azure.com/")
P("e con gpt-6-luna come deployment",
  app.azure_deployments == "gpt-6-luna")
P("la chiave NON è nel codice: senza ambiente né secrets il campo resta vuoto",
  app.api_key == "" and "AZURE_OPENAI_API_KEY =" not in open(QUI_TEST / "app.py").read())
import os as _os
_os.environ["AZURE_DEPLOYMENT"] = "gpt-5.6-luna, gpt-6-luna"
P("una variabile d'ambiente vince sul valore predefinito",
  app.impostazione("AZURE_OPENAI_DEPLOYMENT", "AZURE_DEPLOYMENT", predefinito="gpt-6-luna")
  == "gpt-5.6-luna, gpt-6-luna")
del _os.environ["AZURE_DEPLOYMENT"]
P("senza ambiente né secrets si usa il predefinito, senza errori",
  app.impostazione("NESSUNA_VARIABILE_COSI", predefinito="x") == "x")
