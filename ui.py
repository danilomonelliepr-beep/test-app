...
.testata img { display: block; width: 100%; height: auto;
...
BANNER = Path(__file__).parent / "assets" / "banner-lake.svg"


@lru_cache(maxsize=1)
def _banner_incorporato() -> str:
    """Il banner come `data:` dentro la pagina.

    È un SVG e non un'immagine: pesa due chilobyte invece di novanta, resta
    nitido su qualunque schermo e a qualunque ingrandimento, e usa gli stessi
    colori esatti del tema qui sopra — se l'accento cambia, il banner si
    allinea cambiando una riga invece di rigenerare un file.

    Incorporato e non servito come file perché così viaggia con l'HTML della
    testata e non dipende da come Streamlit espone la cartella dei file
    statici, che cambia fra le versioni. `lru_cache`: si legge e si codifica
    una volta sola, non a ogni riesecuzione della pagina."""
    try:
        dati = base64.b64encode(BANNER.read_bytes()).decode("ascii")
        return f"data:image/svg+xml;base64,{dati}"
    except Exception:
        return ""
...
def testata(titolo: str, compito: str, marcatori: Optional[List[str]] = None,
    ...
    immagine = _banner_incorporato()
    if immagine:
        intestazione = f'<img src="{immagine}" alt="{_e(titolo)}">'
    else:
        intestazione = f'<h1>{_con_iniziali_in_evidenza(titolo) if sigla else _e(titolo)}</h1>'
    st.markdown(
        f'<div class="testata">{intestazione}'
