...
print("\nVersioni dei moduli (devono coincidere)")
try:
    import re as _re
    versioni = {}
    for nome in ["app.py", "model_chain.py", "contract.py", "diagrams.py",
                 "mermaid_render.py", "exporter.py", "ui.py"]:
        m = _re.search(r'__version__ = "([^"]+)"', (QUI / nome).read_text("utf-8"))
        versioni[nome] = m.group(1) if m else "assente"
    diverse = {n: v for n, v in versioni.items() if v != versioni.get("app.py")}
    esito(not diverse, "tutti i moduli alla versione " + versioni.get("app.py", "?"),
          "file di versioni diverse: " + ", ".join(f"{n} ({v})" for n, v in diverse.items())
          + " — aggiorna tutta la cartella dallo stesso pacchetto, non i singoli file")
except Exception as e:  # noqa: BLE001
    esito(False, f"lettura delle versioni: {e}")
